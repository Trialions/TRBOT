...
def fetch_klines(symbol, interval, start_ms, end_ms):
    ...
    while current < end_ms:
        try:
            ...
            log_error(f"fetch_klines {symbol}: {e}")
            break
    return candles


def get_candles(symbol, interval, days, start_ms, end_ms,
                start_date=None, end_date=None):
    cached = _load_cache(symbol, interval, days, start_date, end_date)
    if cached is not None:
        return cached
    candles = fetch_klines(symbol, interval, start_ms, end_ms)
    if candles:
        _save_cache(symbol, interval, days, candles, start_date, end_date)
    return candles
...
def _max_consecutive(trades: list, win: bool) -> int:
    """Ardışık kazanç (win=True) veya kayıp (win=False) sayısı."""
    key    = lambda t: t["net_pnl"] > 0
    target = win
    best = cur = 0
    for t in trades:
        if key(t) == target:
...
def _sharpe(equity_curve: list, risk_free: float = 0.0) -> float:
    """
    Basit Sharpe oranı (günlük getirilerden).
    equity_curve: [(timestamp_ms, equity), ...] listesi
    """
    ...
    if std_r == 0:
        return 0.0
    return round((mean_r - risk_free / 252) / std_r * math.sqrt(252), 3)
...
class Backtester:
    def __init__(self, cfg: dict):
        ...
        # Equity curve: [(open_time_ms, equity), ...]
        self.equity_curve: list = [(0, self.starting_equity)]
    ...
    def step(self, symbol, candle, prices, highs, lows, volumes):
        ...
        self.open_positions[symbol] = {
            ...
            "ts_open":    ts_sec,  "ts_open_ms": ts_ms,
            "score":      score,
        }
...
def generate_report(trades, starting_equity, final_equity,
    ...
    max_gain    = max((t["net_pnl"] for t in trades), default=0)
    max_loss    = min((t["net_pnl"] for t in trades), default=0)
    avg_hold    = sum(t["hold_min"] for t in trades) / total
    total_ret   = (final_equity - starting_equity) / starting_equity * 100

    max_dd      = _max_drawdown(equity_curve)
    sharpe      = _sharpe(equity_curve)
    recovery    = round(net_pnl / (max_dd * starting_equity / 100), 2) if max_dd > 0 else "∞"
    consec_win  = _max_consecutive(trades, win=True)
    consec_loss = _max_consecutive(trades, win=False)
    ...
    for t in trades:
        coin_pnl[t["symbol"]] += t["net_pnl"]
    top_winners = sorted(coin_pnl.items(), key=lambda x: -x[1])[:5]
    top_losers  = sorted(coin_pnl.items(), key=lambda x:  x[1])[:5]

    sep = "=" * 60
    hdr = f"  {label}" if label else "  BACKTEST SONUÇLARI"
    print(f"\n{sep}\n{hdr}\n{sep}")
...
# ──────────────────────────────────────────────
# Parametre Optimizasyonu (Grid Search + OOS)
# ──────────────────────────────────────────────
PARAM_GRID = {
...
def _run_timeline(bt: Backtester, timeline: list, all_candles: dict,
                  window: int = 500):
    """Verilen timeline üzerinde backtester'ı çalıştırır, last_prices döner."""
...
def run_parameter_search(symbols, interval, days, base_cfg,
                         all_candles, start_ms, save_best_to=None,
                         oos_split=1.0):
    """
    PARAM_GRID üzerinde tüm kombinasyonları dener.
    oos_split < 1.0 ise: ilk oos_split oranı train, kalanı OOS test olarak kullanılır.
    En iyi konfigürasyonu döner; save_best_to verilmişse yaml'a yazar.
    """
    from itertools import product

    # ── Zaman eksenini oluştur ve böl ─────────────────────────
    timeline = []
    ...
    split_date = (datetime.utcfromtimestamp(timeline[split_idx][0] / 1000)
                  .strftime("%Y-%m-%d") if test_tl else "?")
    ...
    # ── Train: tüm kombinasyonları dene ───────────────────────
    results = []
    for i, combo in enumerate(combos, 1):
        ...
        print(f"  [{i:3}/{total}] {bar:<30}  "
              ...
              f"sshort={params['score_short_open']} "
    ...
    def score_fn(r):
        penalty = 0
        if r["win_rate"] < 40: penalty += 1000
        if r["max_dd"]   > 20: penalty += 500
        return penalty - r["net_pnl"]

    results.sort(key=score_fn)

    print(f"\n{sep}")
    print(f"  EN İYİ 5 KOMBİNASYON (TRAIN)")
    print(sep)
    for rank, r in enumerate(results[:5], 1):
        p = r["params"]
        print(f"  #{rank}  slong={p['score_long_open']:2.0f}  "
              f"sshort={p['score_short_open']:2.0f}  "
              f"sl={p['hard_stop_pct']:.1f}  "
              f"tp={p['take_profit_min_pct']:.1f}  "
              f"→  WR=%{r['win_rate']:.0f}  "
              f"PnL=${r['net_pnl']:+.0f}  "
              f"DD=%{r['max_dd']:.1f}  "
    ...
    if test_tl:
        print(f"\n{sep}")
        print(f"  OUT-OF-SAMPLE TEST  (son %{int((1 - oos_split) * 100)} veri)")
        print(sep)

        cfg_best = _build_cfg_variant(base_cfg, best["params"])
        bt_oos   = Backtester(cfg_best)

        # Train'in son WINDOW mumunu buffer'a ekle (sıcak başlangıç)
        from collections import deque
        WINDOW    = 500
        pre_buf   = {s: deque(maxlen=WINDOW) for s in all_candles}
        h_pre_buf = {s: deque(maxlen=WINDOW) for s in all_candles}
        l_pre_buf = {s: deque(maxlen=WINDOW) for s in all_candles}
        v_pre_buf = {s: deque(maxlen=WINDOW) for s in all_candles}

        for ts, sym, candle in train_tl[-WINDOW:]:
            pre_buf[sym].append(candle["close"])
            h_pre_buf[sym].append(candle["high"])
            l_pre_buf[sym].append(candle["low"])
            v_pre_buf[sym].append(candle["volume"])

        last_prices = {}
        for ts, sym, candle in test_tl:
            pre_buf[sym].append(candle["close"])
            h_pre_buf[sym].append(candle["high"])
            l_pre_buf[sym].append(candle["low"])
            v_pre_buf[sym].append(candle["volume"])
            last_prices[sym] = candle["close"]
            if len(pre_buf[sym]) >= 50:
                bt_oos.step(sym, candle,
                            list(pre_buf[sym]),
                            list(h_pre_buf[sym]),
                            list(l_pre_buf[sym]),
                            list(v_pre_buf[sym]))

        bt_oos.force_close_all(last_prices)

        oos_t = bt_oos.trades
        if oos_t:
            oos_wins = sum(1 for x in oos_t if x["net_pnl"] > 0)
            oos_wr   = oos_wins / len(oos_t) * 100
            oos_net  = sum(x["net_pnl"] for x in oos_t)
            oos_dd   = _max_drawdown(bt_oos.equity_curve)
            oos_sh   = _sharpe(bt_oos.equity_curve)

            print(f"  İşlem Sayısı  : {len(oos_t)}")
            print(f"  Kazanma Oranı : %{oos_wr:.1f}  "
                  f"(Train: %{best['win_rate']:.1f})")
            print(f"  Net PnL       : ${oos_net:+.2f}  "
                  f"(Train: ${best['net_pnl']:+.2f})")
            print(f"  Max Drawdown  : %{oos_dd:.2f}  "
                  f"(Train: %{best['max_dd']:.2f})")
            print(f"  Sharpe        : {oos_sh:.2f}  "
                  f"(Train: {best['sharpe']:.2f})")

            wr_drop = best["win_rate"] - oos_wr
            if wr_drop > 15:
                print(f"\n  ⚠ UYARI: Win rate train'den %{wr_drop:.0f} düştü — "
                      f"overfitting riski yüksek!")
            elif wr_drop > 8:
                print(f"\n  ~ DİKKAT: Win rate %{wr_drop:.0f} düştü — "
                      f"parametreleri temkinli kullan.")
            else:
                print(f"\n  ✓ OOS tutarlı — train/test farkı %{wr_drop:.0f}")

            best["oos"] = {
                "win_rate": round(oos_wr, 1),
                "net_pnl":  round(oos_net, 2),
                "max_dd":   oos_dd,
                "sharpe":   oos_sh,
                "trades":   len(oos_t),
            }
        else:
            print("  [UYARI] OOS döneminde hiç işlem oluşmadı.")
        print(sep)

    print(f"\n  ✓ En iyi parametreler (train):")
    ...
    if save_best_to:
        try:
            ...
            p = best["params"]
            cfg_yaml.setdefault("thresholds", {})["score_long_open"]    = p["score_long_open"]
            cfg_yaml.setdefault("thresholds", {})["score_short_open"]   = p["score_short_open"]
            cfg_yaml.setdefault("risk", {})["hard_stop_pct"]            = p["hard_stop_pct"]
            cfg_yaml.setdefault("risk", {})["take_profit_min_pct"]      = p["take_profit_min_pct"]
            with open(save_best_to, "w", encoding="utf-8") as f:
                yaml.dump(cfg_yaml, f, allow_unicode=True, default_flow_style=False)
            print(f"\n  ✓ En iyi parametreler {save_best_to} dosyasına yazıldı.")
        except Exception as e:
...
def run_backtest(symbols, interval, days, cfg, out_dir,
    ...
    print(f"\nBacktest {'(OPTİMİZASYON MODU)' if optimize else 'Başlıyor'}"
          f"{' + OOS' if oos else ''}")
    print(f"  Semboller  : {len(symbols)} adet")
    print(f"  Interval   : {interval}")
    print(f"  Süre       : Son {days} gün")
    print(f"  Başlangıç  : {datetime.utcfromtimestamp(start_ms/1000).strftime('%Y-%m-%d')}")
    print(f"  Bitiş      : {datetime.utcfromtimestamp(end_ms/1000).strftime('%Y-%m-%d')}")
    ...
    # ── Normal backtest ───────────────────────────────────────
    print(f"\n  Zaman ekseni oluşturuluyor...")
    timeline = []
    for sym, candles in all_candles.items():
        for c in candles:
            timeline.append((c["open_time"], sym, c))
    timeline.sort(key=lambda x: x[0])
    print(f"  Toplam {len(timeline):,} mum adımı\n")
    ...
    for ts, sym, candle in timeline:
        ...
        if processed % 50000 == 0:
            ...
            print(f"  İlerleme: %{pct:.1f} — Açık: {len(bt.open_positions)} "
                  f"İşlem: {len(bt.trades)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--optimize",    action="store_true",
                        help="Parametre grid search yap")
    parser.add_argument("--save-config", type=str,   default="",
                        help="En iyi parametreleri bu yaml'a yaz")
    parser.add_argument("--oos",         action="store_true",
                        help="Out-of-sample test: ilk %%70 optimize, son %%30 test")
    ...
    try:
        ...
        print("[HATA] config_online.yaml bulunamadı!"); exit(1)
    ...
    if args.symbols:
        ...
        try:
            ...
            print("[HATA] symbols_top70.json bulunamadı!"); exit(1)
